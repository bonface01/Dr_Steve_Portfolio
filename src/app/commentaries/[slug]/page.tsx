    ...
    <PageTransition>
      <article className="bg-surface pt-28">
        <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
          <p className="text-xs font-bold uppercase tracking-[0.28em] text-royal">
            {post.category} Commentary
          </p>
          <h1 className="mt-5 font-heading text-5xl leading-tight text-navy sm:text-7xl">
            {post.title}
          </h1>
        ...
        <div className="relative mx-auto aspect-[16/8] max-w-7xl overflow-hidden rounded-[8px]">
          <MaybeImage src={post.coverImage} alt={post.title} fill priority className="object-cover" />
        </div>
      ...
      <Section eyebrow="Related Commentaries" title="Continue reading.">
        <div className="grid gap-6 md:grid-cols-3">
          {related.map((item) => (
            <BlogCard key={item.id} post={item} hrefPrefix="/commentaries" />
          ))}
        </div>
      </Section>
    </PageTransition>
