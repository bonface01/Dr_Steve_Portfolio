...
type Tab = "overview" | "blog" | "events" | "gallery" | "comments";
...
export function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  async function savePost(event: FormEvent<HTMLFormElement>) {
    ...
    const next = {
      ...
      coverImage: postDraft.coverImage || "https://images.unsplash.com/photo-1517048676732-d65bc937f952?auto=format&fit=crop&w=1400&q=80"
    };
  ...
  async function saveEvent(event: FormEvent<HTMLFormElement>) {
    ...
    const next = {
      ...
      coverImage: eventDraft.coverImage || "https://images.unsplash.com/photo-1515187029135-18ee286d815b?auto=format&fit=crop&w=1400&q=80"
    };
  ...
  async function saveGallery(event: FormEvent<HTMLFormElement>) {
    ...
    const next = {
      ...galleryDraft,
      id,
      image: galleryDraft.image || "https://images.unsplash.com/photo-1515168833906-d2a3b82b1a48?auto=format&fit=crop&w=900&q=80"
    };
...
function PreviewImage({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="relative mt-4 aspect-[16/9] overflow-hidden rounded-[8px]">
      <Image src={src} alt={alt} fill className="object-cover" unoptimized={src.startsWith("data:")} />
    </div>
  );
}
